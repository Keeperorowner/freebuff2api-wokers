...
# DNS 必须自带：容器里没有这段时 mihomo 退回容器系统 DNS（Docker Desktop →
# 宿主机物理网卡 DNS = 国内污染 DNS），节点域名被解析到错误 IP → 全部节点
# 表现为"不可用"。宿主机装的 Clash 能用是因为它自带 fake-ip + DoH 配置。
# 方案与 Clash Verge 默认一致：境内污染规避 DoH 解析节点域名。
dns:
  enable: true
  ...
  fake-ip-filter:
    ...
    - "time.*.com"
    - "ntp.*.com"
  # DoH 服务器域名引导解析（纯 IP、境内可达）
  default-nameserver:
    - 223.5.5.5
    - 119.29.29.29
  # 节点服务器域名专用：阿里 DNS / DNSPod 的 DoH（防污染），
  # 这是解决"DNS 污染导致节点不可用"的关键一行
